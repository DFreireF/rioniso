...
class Plotters(object):
    def __init__(self, width=15, height=8, dpi=300, fs = 14):
        ...
        self._set_fig(width, height, dpi, fs)
    ...
    def update_plot(self, fig = False):
        #if fig is False: 
        #    self._set_fig()
    ...
    def _plot_data(self):
        for i in range(len(self.mean_freq)):
            self.axs.errorbar(
                ...
                fmt='o', picker = True, pickradius = 10, color='black' if self.marker_visibility[i] else 'none'
            )
    ...
    def toggle_visibility(self, index):
        ...
        #self.fig.canvas.draw_idle()  # Redraw the plot to update visibility
        self.update_plot()
